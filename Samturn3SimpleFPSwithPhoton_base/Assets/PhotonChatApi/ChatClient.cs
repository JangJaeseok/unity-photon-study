...
namespace ExitGames.Client.Photon.Chat
{
    ...
    public class ChatClient : IPhotonPeerListener
    {
		...
		public bool AddFriends(List<string> friends)
        {
            if (!this.CanChat)
            {
                if (this.DebugOut >= DebugLevel.ERROR)
			...
			if (friends.Count > FriendRequestListMax)
            {
                if (this.DebugOut >= DebugLevel.WARNING)
                {
					this.listener.DebugReturn(DebugLevel.WARNING, "AddFriends max list size exceeded: " + friends.Count + " > " + FriendRequestListMax);
                }
                return false;
            }
			//-----------------------------------------------------List 를 [] 변환
			string[] afriends = new string[friends.Count];
			for (int i = 0; i < friends.Count; i++) {
				afriends [i] = friends [i];
			}
			//------------------------------------------------------
            Dictionary<byte, object> parameters = new Dictionary<byte, object>
                {
					{ ChatParameterCode.Friends, afriends },
                };
        ...
        public string GetPrivateChannelNameByUser(string userName)   // 비밀 채널 생성
        {
        ...
        private void HandlePrivateMessageEvent(EventData eventData)   // 비밀 메시지
        {
            //Console.WriteLine(SupportClass.DictionaryToString(eventData.Parameters));
			this.listener.DebugReturn(DebugLevel.ALL, "degug message");
            ...
            if (this.UserId != null && this.UserId.Equals(sender))
            {
                ...
                channelName = this.GetPrivateChannelNameByUser(target);  // 채널 이름 만들기
            }
            else
            {
            ...
            channel.Add(sender, message);  // 채널별 대화 저장 
            this.listener.OnPrivateMessage(sender, message, channelName);
        }

        private void HandleChatMessagesEvent(EventData eventData)   // 일반 메시지
        {
            ...
            channel.Add(senders, messages);    // 채널별 대화 저장
